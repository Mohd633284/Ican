        ...
        <div class="p-3 space-y-3" style="padding-top: 1rem; margin-top: 0; pointer-events: auto !important;">
          ...
          <div class="flex items-center gap-1">
            ...
            <input
              ...
              max="20"
              step="0.1"
              class="w-16 px-1 py-0.5 text-xs border border-black dark:border-gray-600 rounded focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
              style="pointer-events: auto !important;"
            />
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <button
                class="flex items-center gap-3 p-2.5 bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors border border-slate-200 dark:border-slate-600 group text-left"
                :disabled="isExporting"
                @click="handleSaveReceipt(); closeMobileExportPopup(); closeDesktopExportPopup()"
              >
                <span class="text-xl group-hover:scale-110 transition-transform">💾</span>
                <div>
...
export default defineComponent({
  ...
  const showPreview = ref(true);
    ...
    const organizationNameFontSize = computed(() => {
      ...
      let baseSize = 40; // Increased from 40 to 48
      
      // Adjust base size based on name length to prevent cramping
      if (nameLength > 30) {
        baseSize = 35; // Increased from 34 to 42
      } else if (nameLength > 20) {
        baseSize = 36; // Increased from 28 to 36
      }
